...
#####   VALLEY OAK   ##### 
ValleyOak_spatialintersect<- read.csv('~/Data/Modified/Groundwater/ValleyOak_CASGEM_closest1km_SpatialJoin.csv', header=TRUE, sep=",")
ValleyOak_spatialintersect$POLYGON_ID <-as.character(ValleyOak_spatialintersect$POLYGON_ID)
ValleyOak_spatialintersect <- data.frame(ValleyOak_spatialintersect$POLYGON_ID,ValleyOak_spatialintersect$SITE_CODE)
colnames(ValleyOak_spatialintersect)<- c('POLYGON_ID','SITE_CODE')
ValleyOak_spatialintersect[which(ValleyOak_spatialintersect$SITE_CODE==" "),]<-NA
ValleyOak_spatialintersect<- na.omit(ValleyOak_spatialintersect)
ValleyOak_DEM <- read.csv('~/Data/Modified/Groundwater/ValleyOak_DEM.csv')  #Load mean elevation data for each veg polygon
ValleyOak_DEM$POLYGON_ID <-as.character(ValleyOak_DEM$POLYGON_ID)
ValleyOak_DEM$Rowid <- NULL
ValleyOak_DEM$COUNT <- NULL
ValleyOak_DEM$AREA <- NULL
ValleyOak_wells <- left_join(ValleyOak_spatialintersect, ValleyOak_DEM, by= "POLYGON_ID")
colnames(ValleyOak_wells)<- c('POLYGON_ID','SITE_CODE','VEG_Elev')
ValleyOak_wells<-left_join(ValleyOak_wells,CASGEM,by="SITE_CODE")
ValleyOak_wells$DTW <- ValleyOak_wells$VEG_Elev - ValleyOak_wells$WSE_m    #Calculate Depth to Groundwater (meters) for each veg polygon
...
ValleyOak <- SentinelInterp(ValleyOak_NDVI, ValleyOak_wells)

#  Apply Function to calculate monthly data for each veg type
ValleyOak_MonthMedian <- MonthlyData(ValleyOak, ValleyOak_spatialintersect)

#  Apply Function to calculate seasonal monthly data for each veg type
ValleyOak_SeasonalMedian <- SeasonalData(ValleyOak_MonthMedian, ValleyOak_spatialintersect)
...
ValleyOak_MonthMedian$veg <- "Valley Oak"
ValleyOak_SeasonalMedian$veg <- "Valley Oak"
#write.csv(ValleyOak_SeasonalMedian, "~/Data/Modified/Merged/ValleyOak_SeasonalMedian.csv")
